...
async function compress(image: Buffer, opt: CompressOption = {}): Promise<Buffer> {
...
async function extend(main: Buffer, extended: Buffer, opt: ExtendOption = {}): Promise<Buffer> {
...
function install() {
  ...
  parentPort!.on('message', async ({ name, params }: EventData) => {
    let data,
      error = '';
    ...
    parentPort!.postMessage({ name, data, error });
  });
}
